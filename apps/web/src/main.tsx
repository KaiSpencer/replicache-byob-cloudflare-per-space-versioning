...
async function init() {
	...
	function Root() {
		// TODO: You can drive this from something that defines the space, ie an organisation or a specific user etc...
		const [spaceID, _] = useState<string>("Kai");
		...
		useEffect(() => {
			...
			const r = new Replicache({
				name: "chat-user-id-per-space",
				licenseKey,
				pushURL: `${import.meta.env.VITE_API_URL}/api/replicache/push?spaceID=${spaceID}`,
				pullURL: `${import.meta.env.VITE_API_URL}/api/replicache/pull?spaceID=${spaceID}`,
				logLevel: "debug",
				mutators: {
					async createMessage(
						tx: WriteTransaction,
...
function listen(r: Replicache, spaceID: string) {
	...
	channel.bind(`poke:${spaceID}`, async () => {
